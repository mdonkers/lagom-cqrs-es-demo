...
/**
 * This interface defines all the commands that the Greeting entity supports.
 * 
...
public interface GreetingCommand extends Jsonable {
  ...
  @Immutable
  @JsonDeserialize
  public final class UseGreetingMessage implements GreetingCommand, CompressedJsonable, PersistentEntity.ReplyType<Done> {
  ...
  public final class Greeting implements GreetingCommand, PersistentEntity.ReplyType<String> {
    ...
    @JsonCreator
    public Greeting(String name, Optional<String> organization) {
    ...
    public boolean equals(@Nullable Object another) {
      if (this == another)
        return true;
      return another instanceof Greeting && equalTo((Greeting) another);
    }

    private boolean equalTo(Greeting another) {
    ...
    @Override
    public String toString() {
      return MoreObjects.toStringHelper("Greeting").add("name", name).add("organization", organization).toString();
    }
  }
